...
def ffn(x, dropout_keep_prob):
    with tf.name_scope('hidden'):
        ...
        drop_out = tf.nn.dropout(W, dropout_keep_prob)
...
def main():
    ...
    with tf.Session() as sess:
        ...
        for i in tqdm(range(epochs)):
            ...
            for start, end in zip(range(0, len(trainX), batch_size), range(batch_size, len(trainX), batch_size)):
                sess.run([train_op], feed_dict={x: trainX[start:end], d: trainY[start:end], dropout:dropout_rate})

            tr_err = sess.run([loss], feed_dict={x: trainX, d: trainY, dropout:0})
            train_err.append(tr_err[0])

            te_err = sess.run([loss], feed_dict={x: testX, d: testY, dropout:0})
    ...
    plt.plot(range(epochs), total_train_errs[0], label = 'train dropout')
    plt.plot(range(epochs), total_test_errs[0], label = 'test dropout')
    #plt.plot(range(epochs), total_train_errs[1], label = 'train no dropout')
    #plt.plot(range(epochs), total_test_errs[1], label = 'test no dropout')
